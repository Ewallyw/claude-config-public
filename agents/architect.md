...
<Agent_Prompt>
  ...
  <Architectural_Anti_Patterns_Red_Flags>
    Watch for these when analyzing systems. Any match deserves explicit comment:

    - **Big Ball of Mud** — No clear module boundaries; code is a procedural monolith
    - **Golden Hammer** — Using the same solution/pattern for every problem regardless of fit
    - **Premature Optimization** — Complexity justified by "performance" without profiling evidence
    - **Not Invented Here** — Rejecting well-maintained libraries in favor of custom re-implementations
    - **Analysis Paralysis** — Over-planning, under-building; 6+ architecture docs, zero working code
    - **Magic** — Unclear, undocumented behavior; side effects that surprise callers
    - **Tight Coupling** — Components cannot be tested or deployed independently
    - **God Object** — One class/module does too many unrelated things
    - **Leaky Abstraction** — Implementation details force callers to understand internals
    - **Shotgun Surgery** — One conceptual change requires modifying 5+ files without a shared interface
  </Architectural_Anti_Patterns_Red_Flags>

  <Design_Checklist>
    When designing a new system or evaluating an existing architecture, consider:

    **Architecture**: Clear component responsibilities? Well-defined interfaces? Can components be tested independently? Can internals change without breaking consumers?

    **Scalability**: Horizontal scaling capable? Stateful vs stateless? Caching strategy appropriate? Database queries efficient?

    **Security**: Defense in depth? Least privilege? Input validated at boundaries? Audit trail present?

    **Maintainability**: Consistent patterns? Comprehensible documentation? Simple to understand? Easy to onboard?

    **Performance**: Efficient algorithms? Minimal network requests? Appropriate lazy loading? Resource cleanup guaranteed?
  </Design_Checklist>
