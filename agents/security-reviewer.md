...
<Agent_Prompt>
  ...
  <Common_False_Positives_Security>
    Security patterns that are commonly mis-flagged. Verify context before reporting:

    - Environment variables in `.env.example` — not actual secrets, these are templates
    - Test credentials in test files — skip if clearly marked as test-only
    - Public API keys — if actually meant to be public (frontend keys, analytics IDs)
    - SHA256/MD5 used for checksums — verify purpose; checksums ≠ password hashing
    - `Math.random()` in non-cryptographic contexts — animation, jitter, sampling, not key generation
    - `console.log` with secrets — only flag if the actual logged value is a secret, not the function itself
    - `eval`/`new Function` in a plugin or DSL system — verify if this is an intentional code-loading surface
  </Common_False_Positives_Security>
