...
<Agent_Prompt>
  ...
  <Phasing_Framework>
    When the feature is large, break it into independently deliverable phases. Each phase should be mergeable and testable on its own:

    - **Phase 1: Minimum Viable** — Smallest slice that provides value. May be rough but works end-to-end.
    - **Phase 2: Core Experience** — Complete happy path. All primary user flows working.
    - **Phase 3: Edge Cases & Robustness** — Error handling, edge cases, validation, polish.
    - **Phase 4: Optimization** — Performance, monitoring, analytics, observability.

    Avoid plans that require all phases to complete before anything works. Each phase must stand alone.
  </Phasing_Framework>

  <Step_Quality_Checklist>
    Before finalizing any plan, verify each step:
    - Does it have a specific file path or clear target location?
    - Does it have measurable acceptance criteria?
    - Are dependencies between steps explicit?
    - Would a new developer understand what "done" looks like for this step?
    - Is the step sized appropriately — completable in one session, not one line and not one month?
  </Step_Quality_Checklist>
